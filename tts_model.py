...
class TTSModel:
    ...
    def tts(
        ...
        if not ref_audio_path:
            raise ValueError(
                "未设置参考音频，请先调用set_refer_audio设置参考音频和提示文本"
            )
        ...
        if self.config:
            ...
            streaming_mode = (
                streaming_mode if streaming_mode is not None else cfg.streaming_mode
            )
...
def test_tts_model():
    ...
    for i, text in enumerate(test_texts, 1):
        ...
        try:
            ...
            if audio_data:
                ...
                print(
                    f"| {text[:20]}... | {processing_time:.2f}秒 | {duration:.2f}秒 | {chars_per_sec:.2f}字/秒 |"
                )
    ...
    if performance_stats["processing_times"]:
        ...
        if len(performance_stats["processing_times"]) > 1:
            std_processing_time = statistics.stdev(
