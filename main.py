...
class TTSPipeline:
    ...
    async def client_handle(self, message_dict: dict) -> None:
        ...
        if random.random() > self.config.probability.voice_probability and message.message_segment.type != 'tts_text':
            # 概率发送语音
    ...
    def simulate_telephone_voice(self, audio_bytes) -> bytes:
        """
    ...
    async def get_voice_no_stream(self, text: str, platform: str) -> Seg | None:
        ...
        try:
            ...
            if self.config.tts_base_config.post_process:
                # 如果启用了后处理，进行电话语音模拟
                audio_data = self.simulate_telephone_voice(audio_data)
            # 对整个音频数据进行base64编码
